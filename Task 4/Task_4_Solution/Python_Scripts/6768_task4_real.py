...
class Edrone():
	...
	def __init__(self):
		...
		self.thres_duration = rospy.Duration(secs=1)
	...
	#------------------------------------------------------------------------------------------------------------------------
		...
		if ((abs(error[0])<1 and abs(error[1])<1 and abs(error[2])<1) or (rospy.Time.now()-self.last_updated > self.thres_duration)):
			self.taskdone = 1
			self.last_updated = rospy.Time.now()
		else:
			self.taskdone = 0
...
class task_4():
	...
	def setsetpoint(self,waypoint):
		...
		print(self.currentWaypoint, self.e_drone.setpoint)
